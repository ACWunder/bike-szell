...
against 0.549, and an LCC share of 89\,\% in both. They differ mainly in total length,
1{,}810\,km for OGD against 2{,}856\,km for OSM, because OSM's ``bikeable'' definition
admits more calmed streets. The structure that this report is about is therefore robust to
the data choice; only absolute lengths must not be compared across sources.
...
\citet{szell2022} for cross-city comparability. Vienna's synthetic figures therefore
differ slightly between the deep dive and the cross-city run, an effect revisited in
Sec.~\ref{sec:limits}.

\subsection{Structural metrics}

Four indicators (Table~\ref{tab:metrics}) capture connectivity, routing quality and reach.
Let \(G\) have edge set \(E\) with lengths \(\ell_e\); let the largest connected component
(LCC) be \(G_{\mathrm{LCC}}\).
...
number. \emph{Spatial coverage} is the share of the city area within a 500\,m buffer of the
...
spread. Table~\ref{tab:crosscity} reports all four indicators for both the real network (B)
and the optimum (C), together with the spatial overlap, so that each city can be read on
its own and against its benchmark.
...
\begin{figure}[p]
\centering
\begin{minipage}[t]{0.48\linewidth}\centering\includegraphics[width=\linewidth]{map_amsterdam.png}\end{minipage}\hfill
\begin{minipage}[t]{0.48\linewidth}\centering\includegraphics[width=\linewidth]{map_berlin.png}\end{minipage}

\vspace{0.6em}
\begin{minipage}[t]{0.48\linewidth}\centering\includegraphics[width=\linewidth]{map_barcelona.png}\end{minipage}\hfill
\begin{minipage}[t]{0.48\linewidth}\centering\includegraphics[width=\linewidth]{map_oslo.png}\end{minipage}

\vspace{0.6em}
\begin{minipage}[t]{0.48\linewidth}\centering\includegraphics[width=\linewidth]{map_vienna.png}\end{minipage}
...
is, and Vienna already has more kilometres than the optimum.
...
longer or shorter skeleton. Directness and global efficiency are estimated from a random
...
fixed 15\,m buffer, not an absolute planning truth. Finally, without temporal data the
question of planned against opportunistic growth, the third sub-question, can be answered
only indirectly, from the present structure.

\section{Conclusion}

Measured against a grown optimum, Vienna's bicycle network is well distributed but
under-connected, and its safe, protected layer is barely connected at all, with 80\,\% reach
but only 18\,\% connectivity across 2{,}360 fragments. A focused 429\,km, some 43\,\% of the
optimal skeleton and mostly in the outer districts, remains unbuilt. Internationally Vienna
is solid but mid-pack. The evidence points to a clear, low-regret strategy: prioritise
connecting the network, by joining the islands of safe infrastructure and adding the few
strategic links the optimum identifies, over simply extending it. Beyond Vienna, the
contribution is a transparent, reproducible pipeline that turns raw cycling geodata into
comparable structural diagnostics, applicable to any city.
